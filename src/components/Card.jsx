...
const Card = ({theme})=> {
    ...
    return ( 
        ...
        <ArrowLeft size={20} className='icon'/></div>
        <p className='back'>Back</p>
                ...
                <div className="border-links">
                        ...
                        {borders.map(border => (
                            <NavLink className="nav-link"
                             key={border} to={`/country/${border}`}>{border}
                            </NavLink>
            ...
            <p> Currencies:<span>{` ${Object.values(country.currencies).map(currency => `${currency.name}`).join(',')}`}</span></p>
            <p> Language: <span>{` ${Object.values(country.languages)[0]}`}</span></p>
            </div>
